...
def test_build_frame_chain_mostly_true_perimeter_neighbors(described_dataset):
    """Честная метрика (без строгого критерия из ТЗ): доля последовательных
    пар в собранной цепочке, чьи детали действительно соседние по эталонной
    раскладке, плюс покрытие (доля рамочных деталей, вошедших в цепочку).

    История: top_k обхода был 3 — обход застревал (покрытие 10.5% на 600
    деталях), после снятия предела покрытие стало 76.8%, но точность —
    лишь 10% (сравнение профиля y(x)). С ICP-расстоянием формы, фильтром
    «следующая деталь стыкуется стороной вдоль рамки» и запретом раннего
    замыкания на стартовом угле: на этом датасете (140 деталей) покрытие
    ~91%, точность ~80%; на 600 деталях — 90.6% / 77%."""
    ...
    assert coverage >= 0.8, f"frame chain coverage={coverage:.2%} (регрессия покрытия из-за top_k?)"
    ...
    assert precision >= 0.6, f"frame chain precision={precision:.2%} (n={checked})"
