...
def generate_box_art(
    ...
    # Верхний потолок должен покрывать самый крупный допустимый холст
    # (box_max_canvas_px^2), иначе на больших пазлах (много деталей ->
    # большой образец) плотность мелких пятен на единицу площади падает —
    # соседние ячейки сетки становятся неотличимо однотонными именно там,
    # где должна быть текстура, и locate теряет сигнал для сопоставления.
    n_fine = min(30000, max(150, textured_px // 900))
